...
- 1. **Researcher**: This agent is responsible for researching the policy / user needs document(s) uploaded by a user, extracting the key information and providing this to the other members of the swarm.
- 2. **Writer**: This agent is responsible for taking the information provided by the Researcher and using it to generate a first draft of GDS compliant content. It is also responsible for ensuring that the content meets the required standards and guidelines.
- 3. **Critic**: This agent is responsible for reviewing the content generated by the Writer, providing feedback and suggestions for improvement. It has access to the same information as the Writer, but its role is to critically evaluate the content and identify any areas where it may fall short of the required standards.
...
|-------|-------|-----------|
| Manager | Claude Sonnet | Balanced speed & cost for task orchestration |
